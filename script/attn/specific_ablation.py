...
for cell_line, scene in list(itertools.product(cell_lines, scenes)):
    data_root = f'/sharedata/home/daihzh/protein/ESM4SL/data/SLKB/specific/{cell_line}/{scene}'
    if not os.path.exists(data_root):
        continue

    print(f'======================== {cell_line} / {scene} ========================')

    table_path = f'{output_root}/{cell_line}/{scene}_ebd96'  # _1e-4
    ...
    for test_fold in range(5):
        ...
        call(['python', 'train.py', 
              '--config-file', 'esm4sl/configuration/attn/ebd96.yaml',
              '--num-gpus', '1',
              'SEED', str(seed),
